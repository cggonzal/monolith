...
func createSessionFile() error {
	...
	buf.WriteString("var store = sessions.NewCookieStore([]byte(config.SECRET_KEY))\n\n")
...
func createAuthControllerFile() error {
	...
	buf.WriteString("\t\t\"csrf_token\": template.HTML(csrf.GetCSRFTokenForForm(w, r)),\n")
	buf.WriteString("\t\t\"csrf_meta\": template.HTML(csrf.GetCSRFMetaTag(w, r)),\n")
	buf.WriteString("\t}\n")
